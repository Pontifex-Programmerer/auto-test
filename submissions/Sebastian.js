...
const name = 'Sebastian'; // replace with your actual name here too
// Task 1
...
function countWords(string){
    string = string.split(".");
    console.log(string + ": " + stringSplitter(string).length);
    return(string + ": " + stringSplitter(string).length);
}
...
// --------------------------------------------------------------------------------------------
function executeCallback(string, callback){
    try {
    callback(string);
}
    catch {
        console.error(string + " is not a function");
    }
}
