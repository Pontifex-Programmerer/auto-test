...
const name = 'max'; // replace with your actual name here too
// Task 1
...
function isEven (num){
    ...
    // husker ikke hvodan skjekke etter partall generelt så jeg bare la inn eskempler
    // men det virker ikke som om det skjekker av boksen
};
...
function doubleSum(num1, num2){
    let result = multiplyByTwo(num1) + multiplyByTwo(num2);
    return result;
};
// Task 6
...
// --------------------------------------------------------------------------------------------
function decapitalizer(element){
    if( element === String){
        element.toLowerCase()
    } else {
        return("not string");
