...
const name = 'Simon Tijanic'; // replace with your actual name here too
// Task 1
...
// --------------------------------------------------------------------------------------------
 ...
 function sentenceBuilder(...parameter) {
...
function stringSplitter(string) {
    return string.split(" ").filter(wordString => typeof wordString === "string").filter(word => word.length > 0);
 }
...
//function decapitalizer(args...){
    //return args.filter(argumnt => typeof argument === "string").join("=")
//} // er det ikke bare ... ? 

// if args.filter(argumnt=> argument !== "string" return end)
...
// stringen er lengre en lengst, return hvis ikke så  : ? ebteb de n eller lengste
function findLongestWord(strinh) {
    return strinh.split(" ").reduce((longest, current) => current.length > longest.length ? current : longest, "");
 }
...
//function countWords(string) {
  //  let sentences = string.split(/[.!]/); // seperator regex / [ fjerne . og  ] /
  //  sentences.filter(sentence =>
  //      sentence.trim() //også length? også splitter også length? vetta faeb hva i helvete
 //   )
 //   return "denne klarer jeg ikke"
//}
 
function countWords(string) {
    let sentences = string.split(/[.!]/); // seperator regex / [ fjerne . og  ] /
    let count = sentences.filter(sentence =>
...
async function executeCallback(message, callback){
    try {
...
async function serialExecutor(count, callback) {
    try {
